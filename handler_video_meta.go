...
func (cfg *apiConfig) handlerVideoMetaCreate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Couldn't decode parameters, nope", err)
		return
	}
...
func (cfg *apiConfig) handlerVideoMetaDelete(w http.ResponseWriter, r *http.Request) {
	...
	respondWithJSON(w, http.StatusNoContent, "Delete successful")
}
...
func (cfg *apiConfig) handlerVideosRetrieve(w http.ResponseWriter, r *http.Request) {
	...
	for i, video := range videos {
		vid, err := cfg.dbVideoToSignedVideo(video)
		if err != nil {
			respondWithError(w, http.StatusInternalServerError, "failed to presign file", err)
			return
		}
		videos[i] = vid
	}
